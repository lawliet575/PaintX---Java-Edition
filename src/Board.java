...
public Board() { //this board is the white one yahi sari drawings hogi  yessssss
        ...
        System.out.println("Some instructions for file saving and opening;");
        System.out.println("To open any file, Click on file button and then open button to open any file");
        System.out.println("Note that when you open a file all data will be saved in that opened file");
        System.out.println("To create a new file, first click new in file menu and then save and then open the newly created file");
    }
   ...
   @Override
    public void paintComponent(Graphics g) {
        ...
        layers.paintlayers(g);//shape drawing in layers   
    ...
    */
    ...
    public void IsClicked(int x, int y){
        if(!fileButton.getClicked() && !toolbar.winClicked()){ //undo redo here
         int ur=editButton.handleClick(x,y); //edit button clicked
            if(ur==0){ //redo

            }
               if(ur==1){ //undo
                   layers.undo_functionality();
                    repaint();
               }  
    	...
    	if(!editButton.getClicked() && !toolbar.winClicked()){ //filing here 
            fileButton.handleClick(x,y); //file button clicked
            if(fileButton.getcurrentfilename()!=null){
                String temp=fileButton.getcurrentfilename();
                fw=new File_Writing(temp, layers);
                fw.dowrite();
            }
        }
